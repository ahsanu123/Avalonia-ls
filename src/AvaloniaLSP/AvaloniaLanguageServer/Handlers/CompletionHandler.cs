...
public class CompletionHandler : CompletionHandlerBase
{
    ...
    )
    {
        ...
        var ci = new CompletionItem
        {
            ...
            Command = Command.Create(
                ...
                new { repositionCaret = RepositionCaret() }
            ),
        };
        ...
        bool NeedResolve() =>
            request.InsertText!.EndsWith(".") || request.InsertText!.EndsWith("\"");
    ...
    {
        string? text = _workspace.BufferService.GetTextTillPosition(
                ...
                new[]
                {
                    new CompletionItem
                    {
                        ...
                        Documentation = new StringOrMarkupContent(
                            "Build the project to enable code completion"
                        ),
        ...
        var completions = set
            ?.Completions.Where(p => !p.DisplayText.Contains('`'))
    ...
    static CompletionItemKind GetCompletionItemKind(CompletionKind completionKind)
    {
        ...
        {
            _ when name.Contains("Property") || name.Contains("AttachedProperty") =>
                CompletionItemKind.Property,
            _ when name.Contains("Event") => CompletionItemKind.Event,
            _ when name.Contains("Namespace") || name.Contains("VS_XMLNS") =>
