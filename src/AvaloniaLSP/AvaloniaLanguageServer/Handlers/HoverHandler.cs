...
public sealed class HoverHandler : HoverHandlerBase
{
    ...
    {
        _logger.LogInformation("Hover handlers: {Workspace}", clientCapabilities.Workspace);

        return new() { DocumentSelector = _documentSelector };
    }
    ...
    )
    {
        ...
        var hover = new Hover
        {
            Contents = new MarkedStringsOrMarkupContent(
                new MarkupContent { Kind = MarkupKind.Markdown, Value = $"# {DateTime.Now}" }
            ),
        };
